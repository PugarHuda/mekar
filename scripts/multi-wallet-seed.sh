#!/usr/bin/env bash
#
# MEKAR multi-wallet seed — proves the royalty cascade on Galileo by
...
echo "═══════════════════════════════════════════════"
echo "  MEKAR Multi-wallet Seed (v2)"
echo "═══════════════════════════════════════════════"
...
# ─────────────────────────────────────────────────────────────────────
# 2. Fund each wallet with 0.005 OG
# ─────────────────────────────────────────────────────────────────────
echo "[2/9] Fund each wallet with 0.005 OG…"
for name in alice bob carol; do
  if send_and_wait "fund-$name" $DEPLOYER_PRIVATE_KEY \
       ${ADDRESSES[$name]} --value 5000000000000000 >/dev/null; then
    echo "       $name funded"
  else
...
echo ""
echo "View on chain:"
echo "  AgentINFT: https://chainscan-galileo.0g.ai/address/$AGENT_INFT"
echo "  Vault:     https://chainscan-galileo.0g.ai/address/$VAULT"
